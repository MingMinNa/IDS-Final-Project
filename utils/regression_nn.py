...
class RegressionNN(nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_size = [64, 256, 1024, 1024, 512, 64], hidden_layer_cnt = 6):
        ...
        for i in range(1, hidden_layer_cnt):
...
def predict(regression_model, input_features):

    input_features_tensor = torch.tensor(input_features.values, dtype=torch.float32).to(DEVICE)
    regression_model.to(DEVICE)

    predicts = []
    regression_model.eval()
    with torch.inference_mode():
        for i in range(len(input_features_tensor)):
            predict = regression_model(input_features_tensor[i].unsqueeze(0))
