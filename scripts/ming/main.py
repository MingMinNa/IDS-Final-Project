...
def train_models():
    for site in tqdm(const.sitenames):
        ...
        plt.savefig(os.path.join(const.IMG_FOLDER, 'train', f"ensemble_losses({site}).png"))
...
def test_models():
    ...
    for site in tqdm(const.sitenames):
        ...
        img_path = os.path.join(const.IMG_FOLDER, 'test', f"ensemble_error({site}).png")
        plt.tight_layout()
        ...
        img_path = os.path.join(const.IMG_FOLDER, 'test', f"ensemble_predict({site}).png")
        plt.tight_layout()
        plt.savefig(img_path)
